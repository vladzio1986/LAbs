...
void employee::finding()
{
    int size;
    employee_type etype;
    ifstream ff;
    ff.open("EMPLOY.DAT", ios::binary);
    if(!ff)
    {
        cout << "\nUnable open file\n";
        return;
    ...
    while(true)
    {
        ...
        ff.read( (char*)&etype, sizeof(etype) );
        switch(etype)
        {
            case tmanager:
                    arrap[n] = new manager;
